...
import com.example.darkness.KeepUrFund.adapter.TypeRankAdapter;
import com.example.darkness.KeepUrFund.bean.TypeBean;
import com.example.darkness.KeepUrFund.utils.DateUtils;
import com.example.darkness.KeepUrFund.utils.FormatUtils;
import com.example.darkness.KeepUrFund.utils.PieChartUtil;
import com.example.darkness.KeepUrFund.utils.DataUtils;
import com.example.darkness.KeepUrFund.view.CircleImageView;
...
public class MenuTypeFragment extends BaseFragment
    ...
    protected void initEventAndData() {
        ...
        list = new ArrayList<>();
        adapter = new TypeRankAdapter(getActivity(), list);
    ...
    private void setReportData() {
        ...
        if (TYPE) {
            ...
            typeModel = DataUtils.getTallyTypeBeanOut();
        } else {
            centerTitle.setText("总收入");
            centerImg.setImageResource(R.mipmap.tallybook_input);
            typeModel = DataUtils.getTallyTypeBeanIn();
        }
        if (typeModel == null) {
            return;
        }
    ...
    private void setNoteData(int index){
        ...
        list.clear();
        for(int i = 0; i < tMoneyBeanList.get(index).getList().size(); i++){
            list.add(type_name);
        }
        adapter.setRanklist(tMoneyBeanList.get(index).getList());
